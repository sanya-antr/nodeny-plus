...
{
    Doc->template('base')->{document_ready} .= "\n nody.ajax({ a:'ajGraph', domid:'main_block', module:'$cur_module', y_title:'Трафик, мбит/с' });";
    return 1;
}
 ...
 elsif( $make_graph )
{
    my $unikey = Save_webses_data(
        module=>'ajGraph', data=>{ points=>$points, descr=>$descr, module=>ses::cur_module }
    );
